...
void CrowdProcessingCallback(const visualanalysis_msgs::CrowdHeatmapConstPtr& msg, const multidrone_msgs::GimbalStatusConstPtr& gimbal_msg, const multidrone_msgs::CameraStatusConstPtr& camerastatus_msg, const multidrone_msgs::DroneTelemetryConstPtr& dronetelemetry_msg)
{
	...
	cout<< "Timeframe     Timestamp: "<<msg->header.stamp<<endl;
		...
		w=dst.cols *(camerastatus_msg->sensorWidth)*0.001/1920;//width in m according to sensor size width of blackmagic: 12.48*0.001/1920
		h=dst.rows *(camerastatus_msg->sensorHeight)*0.001/1080;//height in m according to sensor size height of blackmagic: 7.02*0.001/1080
		cout<<"w="<<w<<" h="<<h<<endl;
...
void Threshold_Demo( int, void* )
{
    ...
    threshold( src_gray, dst, threshold_value=37, max_BINARY_value,threshold_type=0 );
...
// Open the KML file for writing:
    stringstream filename;
    filename << path <<"//Sample_clipper_result.kml"; //_" << std::setw(4) << std::setfill('0') << kml_clipper_count << ".kml";
    handle.open(filename.str().c_str());
           ...
           for (std::size_t i = 0; i < elements.size(); ++i) {
            ...
            oss << "<Placemark>\n"
                ...
                for (std::size_t j = 0; j < elements[i].size(); j++) {
                     oss << elements[i].at(j).x()/100000000000000000+15<< "," << elements[i].at(j).y()/100000000000000000+10<< "," << elements[i].at(j).z()/100000000000000000<< " \n";
                 }                                                                                                                                        
            oss << "</coordinates>\n"
                //  <<"</LineString>\n";
                << "</LinearRing>\n"
...
void find_contours_and_project(Mat dst, OcTree* octree , ColorOcTree* ctree, double w, double h, double F, double roll, double pitch, double yaw, vector<vector<point3d> > &polygons_coord){
		...
		bool flag=true;
		c_start3 = std::clock();
		direction.x()=-w/2;//-cx;//TODO:direction.x()+(-w/2-cx);//where direction=M*(0,0,-1)
		direction.y()=-h/2;//-cy;//TODO:direction.y()+(-h/2-cy);
		direction.z()=-F;  //-F; //TODO:direction.z()+(-F+F);
